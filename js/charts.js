// File: js/charts.js
// Fungsi: Rendering UI Data Visual (Radar, Donut, Heatmap, Badges)

import { playerState } from './state.js';
import { calculateLevelInfo } from './player.js';

/**
 * ==========================================
 * 1. AUTO-MIGRASI & DATA VALIDATION
 * ==========================================
 */
const migrateOldStats = () => {
    ...
    const radar = playerState.statsRadar;
    // Deteksi skema lama dan konversi dalam memori
...
/**
 * ==========================================
 * 2. RAK TROFI (BADGES) - O(N) DOM Optimization
 * ==========================================
 */
export function updateBadges() {
    try {
        ...
        const streak = parseInt(localStorage.getItem('streakNum'), 10) || 0;
        ...
        const badges = [
            ...
            { id: 'b5', icon: '⚡', name: 'Admin Pusat', desc: 'Stat Sholat > 50', unlocked: radar.pusat >= 50 },
            { id: 'b6', icon: '💰', name: 'Crazy Rich', desc: 'Stat Sedekah > 50', unlocked: radar.derma >= 50 }
        ];

        // Optimasi Ekstrem: Mencegah DOM Reflow dengan memanipulasi string di memori terlebih dahulu
        badgeContainer.innerHTML = badges.map(b => b.unlocked ? `
            <div class="flex flex-col items-center p-3 bg-gradient-to-br from-yellow-50 to-yellow-200 dark:from-yellow-900/40 dark:to-yellow-800/40 rounded-2xl border border-yellow-300 dark:border-yellow-700 shadow-sm transform hover:-translate-y-1 transition duration-300 relative group cursor-pointer min-w-[80px] shrink-0 snap-center">
                ...
                <div class="absolute -top-10 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white text-[10px] font-bold py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition pointer-events-none whitespace-nowrap z-50">${b.desc}</div>
            </div>` : `
        ...
        console.error("Crash dicegah pada Rak Trofi:", error);
    }
}

/**
 * ==========================================
 * 3. SISTEM AURA & CHARTS (Radar & Donut)
 * ==========================================
 */
window.getAuraBadgeUI = function(level, dominantStatName) {
    const statEmojis = { 'Pusat': '🕋', 'Aura': '✨', 'Peka': '👼', 'Sigma': '🗿', 'Derma': '🤝', 'Stoic': '🧊' };
    const baseEmoji = statEmojis[dominantStatName] || '🔥';
    
    // Konfigurasi level dikemas dalam objek evaluasi linear agar tidak terlalu banyak if/else
    let config = { class: 'bg-gray-800 border-gray-600', color: 'text-gray-400', emoji: baseEmoji };

    if (level >= 50) config = { class: 'bg-black border-transparent relative z-10 before:absolute before:-inset-1 before:bg-gradient-to-r before:from-red-500 before:via-yellow-500 before:to-purple-500 before:-z-10 before:animate-spin shadow-[0_0_25px_rgba(239,68,68,0.8)]', color: 'text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-red-500 font-black tracking-widest animate-pulse', emoji: `<span class="inline-block animate-bounce">⚡👑${baseEmoji}👑⚡</span>` };
    else if (level >= 40) config = { class: 'bg-fuchsia-950 border-fuchsia-500 shadow-[0_0_20px_rgba(217,70,239,0.6)]', color: 'text-fuchsia-400 font-black tracking-widest drop-shadow-md', emoji: `🔮${baseEmoji}🔮` };
    else if (level >= 30) config = { class: 'bg-gradient-to-br from-yellow-900 to-yellow-700 border-yellow-400 shadow-xl shadow-yellow-500/50 relative overflow-hidden', color: 'text-yellow-300 font-black', emoji: `<span class="inline-block animate-bounce">👑${baseEmoji}</span>` };
    else if (level >= 20) config = { class: 'bg-slate-800 border-slate-300 shadow-lg shadow-slate-400/50 animate-pulse', color: 'text-slate-200', emoji: `❄️${baseEmoji}❄️` };
    else if (level >= 10) config = { class: 'bg-gray-800 border-amber-700 shadow-lg shadow-amber-900/40', color: 'text-amber-500', emoji: `${baseEmoji}✨` };

    return `
    <div class="rounded-2xl p-2 border-2 flex flex-col items-center justify-center text-center transition-all ${config.class} h-full min-h-[90px]">
        <div class="text-3xl mb-1 drop-shadow-lg">${config.emoji}</div>
        <p class="text-[8px] font-bold uppercase tracking-widest text-gray-300 mb-0.5 opacity-80">Aura Dominan</p>
        <h4 class="text-[10px] ${config.color} uppercase line-clamp-1">${dominantStatName}</h4>
    </div>`;
};
...
export function renderCharts() {
    try {
        ...
        window.radarChartInstance = new Chart(ctxR.getContext('2d'), {
            ...
            data: {
                labels: labels,
                datasets: [{ data: dataPts, backgroundColor: 'rgba(16, 185, 129, 0.2)', borderColor: 'rgba(16, 185, 129, 1)', pointBackgroundColor: '#FBBC05', borderWidth: 1.5, pointRadius: 2 }]
            },
        ...
        const totalBoxes = document.querySelectorAll('.checklist-item, .sholat-item').length;
        const checkedBoxes = document.querySelectorAll('.checklist-item:checked, .sholat-item:checked').length;
        const percentVal = totalBoxes === 0 ? 0 : Math.floor((checkedBoxes / totalBoxes) * 100);
        ...
        console.error("Crash dicegah pada Chart Rendering:", error);
    }
}

/**
 * ==========================================
 * 4. HEATMAP (JEJAK AKTIVITAS 7 HARI) - Robust Parsing
 * ==========================================
 */
window.activityHistory = (() => {
    try { 
        const parsedData = JSON.parse(localStorage.getItem('activityHistory'));
        return Array.isArray(parsedData) ? parsedData : []; 
    } catch(e) { 
        console.error("[Heatmap Diagnostic] Fail init data, reseting.", e);
        return []; 
    }
})();
...
window.updateHeatmap = function() {
    ...
    if (!Array.isArray(window.activityHistory)) window.activityHistory = [];

    // Pre-kalkulasi 7 hari terakhir
    ...
    // DOM Batch Rendering untuk optimasi performa
    const htmlBuffer = last7Days.map(dateStr => {
        try {
            const record = window.activityHistory.find(r => r && r.date === dateStr);
            ...
            return `<div onclick="alert('Kasta Hari Ini: ${tooltip}')" class="flex flex-col items-center gap-1 cursor-pointer hover:scale-110 transition">
                        <div class="w-8 h-8 rounded-lg ${colorClass} transition-colors duration-500" title="${tooltip}"></div>
                        <span class="text-[9px] font-bold text-gray-400">${dayName}</span>
                    </div>`;
        } catch (err) {
            console.error("[Heatmap Diagnostic] Node rendering error.", err);
            return ''; // Cegah tampilan rusak jika 1 node gagal
        }
    });

    // Injeksi tunggal ke DOM
...
window.logActivityForHeatmap = function() {
    ...
    let recordIndex = window.activityHistory.findIndex(r => r && r.date === dateStr);
    
    if (recordIndex >= 0) {
        window.activityHistory[recordIndex].count += 1;
    } else {
        window.activityHistory.push({ date: dateStr, count: 1 });
    }
