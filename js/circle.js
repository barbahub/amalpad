    ...
    if (spendKoin(amount)) {
        ...
        if (window.circleExp >= window.circleTarget) {
            ...
            if(typeof window.pushToLiveFeed === 'function') {
                window.pushToLiveFeed(playerState.name || "Kamu", `Menyumbang Koin ke Circle`, 'donate', amount, playerState.equippedItems);
            }
        }
...
window.pushToLiveFeed = function(title, desc, type = 'task', extraValue = null, equipped = null) {
    ...
    // Jika tidak dioper, ambil milik user sendiri dari state
    if(!equipped) {
        equipped = playerState.equippedItems || {};
    }
...
window.showUserPopup = function(name, lastQuest, initial, simulatedExp = null) {
    try {
        ...
        let targetExp = simulatedExp !== null ? simulatedExp : playerState.exp;
...
// Injeksi awal agar tidak kosong
setTimeout(() => {
    const feedContainer = document.getElementById('live-feed-container');
...
function initRekapSystem() {
    ...
    if (isEndOfMonth) {
        ...
        btnRekap.addEventListener('click', () => {
            // Dummy bypass check untuk preview standalone
            ...
            const userExp = playerState.exp || 0;
