...
const bioInputFire = document.getElementById('user-bio');
const quoteInputFire = document.getElementById('user-quote');
...
if(btnEditProfile) {
    btnEditProfile.addEventListener('click', async () => {
        ...
        let newName = prompt("Masukkan Username Baru:", nameInputFire ? nameInputFire.value : "");
...
window.fetchCircleData = async (circleId) => {
    ...
    try {
        ...
        if(snap.exists()) {
            ...
            if(joinedUI) joinedUI.classList.remove('hidden');
...
onAuthStateChanged(auth, async (user) => {
    if (user && !user.isAnonymous) {
        ...
        const gmt = document.getElementById('global-mission-title');
        if(gmt && typeof misiSaatIni !== 'undefined') gmt.innerText = misiSaatIni.title;
        window.fetchLeaderboard(); window.fetchGlobalProgress();
    }
});
